...
void main()
{
	...
	gl_FragColor = vec4(watercolor.rgb * (0.9 + dot(normal, lightdir) * 0.1) + pow(dot(normalize(playerpos - pos), dot(lightdir, normal) * 2 * normal - lightdir), 30) * vec3(0.7, 0.7, 0.65), watercolor.a);
}